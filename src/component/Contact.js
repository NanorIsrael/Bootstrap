...
class Contact extends Component {
    ...
    validate = (firstname,lastname,telnum,email)=>{
        const errors ={
            ...
            const reg = /^\d+$/;
            if(this.state.touched.lastname && !reg.test(telnum) )
            errors.telnum = "Tel. number can contain only numbers";

            if(this.state.touched.lastname && email.split('').filter((x ) => x === '@').length !==1 )
...
handleInputChange=(event)=>{
 ...
 handleSubmit=(event)=>{
     ...
     alert('Please form field can not be empty');
    ...
    // Context
    // // Code-Splitting
    context="button" className="btn btn-success" href="mailto:confusion@food.net"><i className="fa fa-envelope-o"></i> Email</a>
                        </div>
                    </div>
                </div>
                <div className="row  row-content">
